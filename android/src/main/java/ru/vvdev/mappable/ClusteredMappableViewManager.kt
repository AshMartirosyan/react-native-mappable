...
import ru.vvdev.mappable.view.ClusteredMappableView
...
class ClusteredMappableViewManager internal constructor() : ViewGroupManager<ClusteredMappableView>() {
    ...
    fun setClusteredMarkers(view: View, points: ReadableArray) {
        castToMappableMapView(view).setClusteredMarkers(points.toArrayList())
    }

    @ReactProp(name = "clusterColor")
    fun setClusterColor(view: View, color: Int) {
        castToMappableMapView(view).setClustersColor(color)
    }

    private fun castToMappableMapView(view: View): ClusteredMappableView {
        return view as ClusteredMappableView
    }

    @Nonnull
    public override fun createViewInstance(@Nonnull context: ThemedReactContext): ClusteredMappableView {
        val view = ClusteredMappableView(context)
    ...
    fun setFollowUser(view: View, follow: Boolean?) {
        castToMappableMapView(view).setFollowUser(follow!!)
    }
    ...
    override fun addView(parent: ClusteredMappableView, child: View, index: Int) {
        parent.addFeature(child, index)
        super.addView(parent, child, index)
    }

    override fun removeViewAt(parent: ClusteredMappableView, index: Int) {
        parent.removeChild(index)
        super.removeViewAt(parent, index)
    }

    companion object {
        const val REACT_CLASS: String = "ClusteredMappableView"

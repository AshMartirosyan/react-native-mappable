...
import world.mappable.mapkit.directions.driving.DrivingSection
...
import world.mappable.mapkit.map.MapLoadStatistics
import world.mappable.mapkit.map.MapLoadedListener
...
import world.mappable.mapkit.transport.masstransit.Weight
...
//        mapWindow.setMaxFps(maxFps)
    ...
    override fun onCameraPositionChanged(
        map: world.mappable.mapkit.map.Map,
        cameraPosition: CameraPosition,
        reason: CameraUpdateReason,
        finished: Boolean
    ) {
    ...
    override fun onMapTap(map: world.mappable.mapkit.map.Map, point: Point) {
    ...
    override fun onMapLongTap(map: world.mappable.mapkit.map.Map, point: Point) {
        val data = Arguments.createMap()
        data.putDouble("lat", point.latitude)
        data.putDouble("lon", point.longitude)
        val reactContext = context as ReactContext

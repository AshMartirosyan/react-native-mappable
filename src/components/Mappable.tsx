...
export interface MappableMapProps extends ViewProps {
  ...
  onActiveIndoorLevelChanged?: (
    event: NativeSyntheticEvent<Pick<IndoorPlan, "activeLevelId">>
  ) => void;
...
const MappableMapNativeComponent =
  requireNativeComponent<MappableMapProps>("MappableView");
...
export class MappableMap extends React.Component<MappableMapProps> {
  ...
  public static setLocale(locale: string): Promise<void> {
    return new Promise((resolve, reject) => {
      NativeMappableModule.setLocale(
        locale,
        () => resolve(),
  ...
  public static getLocale(): Promise<string> {
    return new Promise((resolve, reject) => {
      NativeMappableModule.getLocale(
  ...
  public static resetLocale(): Promise<void> {
    return new Promise((resolve, reject) => {
      NativeMappableModule.resetLocale(
        () => resolve(),
        (err: string) => reject(new Error(err))
      );
    });
  ...
  public setIndoorLevel(indoorLevelId: string) {
    UIManager.dispatchViewManagerCommand(
      findNodeHandle(this),
      this.getCommand("setIndoorLevel"),
      [indoorLevelId]
    );
  }
  ...
  private getCommand(cmd: string): any {
    return Platform.OS === "ios"
      ? UIManager.getViewManagerConfig("MappableView").Commands[cmd]
      : cmd;
  }
  ...
  private getProps() {
    ...
    processColorProps(
      props,
      "userLocationAccuracyFillColor" as keyof MappableMapProps
    );
    processColorProps(
      props,
      "userLocationAccuracyStrokeColor" as keyof MappableMapProps
    );
  ...
  render() {
    return <MappableMapNativeComponent {...this.getProps()} ref={this.map} />;
  }
}
